...
handleMessage('ELECTRON_GUEST_VIEW_MANAGER_CALL', async function (event, guestInstanceId, method, args) {
  const guest = getGuestForWebContents(guestInstanceId, event.sender)
  if (!asyncMethods.has(method)) {
    throw new Error(`Invalid method: ${method}`)
  }

  return serialize(await guest[method](...args))
})
...
handleMessageSync('ELECTRON_GUEST_VIEW_MANAGER_CALL', function (event, guestInstanceId, method, args) {
  ...
  return serialize(guest[method](...args))
})
